...
const { sendOTP, sendRegistrationEmail } = require("../utils/emailService");
...
exports.registerFaculty = async (req, res) => {
  try {
    ...
    try {
      await sendRegistrationEmail(newFaculty.email, newFaculty.name);
    } catch (emailErr) {
...
exports.registerHOD = async (req, res) => {
  try {
    ...
    // Send registration notification email (non-blocking)
    try {
      await sendRegistrationEmail(newHOD.email, newHOD.name);
    } catch (emailErr) {
      console.error("Registration email failed:", emailErr);
    }
